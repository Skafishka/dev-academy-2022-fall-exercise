...
const Journeys = (props) => {
  if (props.month === 1) {
    return (
      <>
        <h4>Filter by station departure name: </h4> <input value={props.showFiltered} onChange={props.handleFilterChange}/>
        ...
        {props.journeysMay.filter(station => station["Departure station name"].toLowerCase()
          ...
          .map((value, id) => (
            <li key={id.toString()}>
              <b>Departure: </b> {value["Departure station name"]} <b>Return: </b>{value["Return station name"]} <b>Duration: </b> {moment.duration(value["Duration (sec)"], "seconds").format("mm:ss")} min <b>Distance: </b> {value["Covered distance (m)"] / 1000} km
            </li>
          ))
        }
      </>
